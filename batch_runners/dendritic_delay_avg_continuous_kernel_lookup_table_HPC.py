...
def get_parameters_lookup_table(theta_values, gradient_values, intensity_values, fraction_nmda_values,
                                fraction_gaba_a_values, fraction_ex_values, cell_id_values, soma_depth_values):

    param_columns = ['theta', 'gradient', 'intensity', 'fraction_nmda', 'fraction_gaba_a', 'fraction_ex', 'cell_id',
                     'soma_depth']
    free_param_length = len(theta_values) * len(gradient_values) * len(intensity_values) * len(fraction_nmda_values) * \
                        len(fraction_gaba_a_values) * len(fraction_ex_values)
    total_num_simulations = free_param_length * len(cell_id_values) * len(soma_depth_values)
    params = np.zeros(shape=(total_num_simulations, len(param_columns)), dtype=float)
    free_index_mask = np.zeros(shape=(total_num_simulations,), dtype=int)
    free_params = np.zeros(shape=(free_param_length, 6), dtype=float)
    ...
    for theta_value in theta_values:
        for gradient_value in gradient_values:
            for intensity_value in intensity_values:
                for fraction_nmda_value in fraction_nmda_values:
                    for fraction_gaba_a_value in fraction_gaba_a_values:
                        for fraction_ex_value in fraction_ex_values:
                            free_params[free_param_counter, :] = np.array(
                                [theta_value, gradient_value, intensity_value, fraction_nmda_value,
                                 fraction_gaba_a_value, fraction_ex_value]
                            )
                            # Loop over cell morphologies and depths for each set of 6 free parameters
                            for cell_id_value in cell_id_values:
                                for soma_depth_value in soma_depth_values:
                                    params[row, :] = np.array(
                                        [theta_value, gradient_value, intensity_value, fraction_nmda_value,
                                         fraction_gaba_a_value, fraction_ex_value, cell_id_value, soma_depth_value]
                                    )
                                    free_index_mask[row] = free_param_counter
                                    row += 1
                            free_param_counter += 1

    return params, param_columns, free_params, param_columns[:6], free_index_mask
...
if __name__ == '__main__':
    ...
    parameter_file = save_path.joinpath(
        f'dendritic_delay_avg_continuous_params_lookup_table_NMDA_0p5_GABAa_0p9_EX_0p5.hdf5'
    )
    sim_params, columns, params_free, columns_free, index_mask = get_parameters_lookup_table(
        theta_values=theta_vals,
        gradient_values=np.array([0]),
        intensity_values=intensity_vals[::2],
        fraction_nmda_values=np.array([0.5]),
        fraction_gaba_a_values=np.array([0.9]),
        fraction_ex_values=np.array([0.5]),
        cell_id_values=cell_ids,
        soma_depth_values=z_sample_depths,
    )

    if (not parameter_file.is_file()) and (process_idx == 0):
        with h5py.File(parameter_file, 'w') as f_param:
            all_pars = f_param.create_dataset(name='params', data=sim_params, dtype='float32', compression=8,
                                              chunks=(1, sim_params.shape[1]))
            all_pars.attrs.create(name='columns', data=columns)
            free_pars = f_param.create_dataset(name='free_params', data=params_free, dtype='float32', compression=8,
                                               chunks=(1, params_free.shape[1]))
            free_pars.attrs.create(name='columns', data=columns_free)
            f_param.create_dataset(name='free_index_mask', data=index_mask, dtype='int32')

    col_enum = Enum('Columns', [col_label for col_label in columns], start=0)
