...
if __name__ == '__main__':
    ...
    label_suffix = param_path.stem[param_path.stem.find('NMDA'):]
    save_path = pathlib.Path(args.save_path)
    chunk_folder = pathlib.Path(args.chunk_path)
    save_file = save_path.joinpath(f'dendritic_delay_current_{label_suffix}.hdf5')
    save_file_raw = save_path.joinpath(f'raw_dendritic_delay_current_{label_suffix}.hdf5')
    save_file_csv = save_path.joinpath(f'dendritic_delay_current_{label_suffix}.csv')
    save_file_csv_time = save_path.joinpath(f'time_axis_{label_suffix}.csv')
    save_file_raw_csv = save_path.joinpath(f'raw_dendritic_delay_current_{label_suffix}.csv')
    param_file_raw_csv = save_path.joinpath(f'dendritic_delay_raw_params_{label_suffix}.csv')
    param_file_csv = save_path.joinpath(f'dendritic_delay_params_{label_suffix}.csv')
    # ################################ Load parameter file #############################################################
    ...
    with h5py.File(param_path, 'r') as f_param:
        ...
        cols_free = f_param['free_params'].attrs.__getitem__('columns')
    ...
    if not save_file.is_file():
        ...
        with open(param_file_csv, 'w') as f_param_csv:
            writer = csv.writer(f_param_csv)
            writer.writerow(cols_free)
            writer.writerows(unique_free_params)

        with open(param_file_raw_csv, 'w') as f_param_raw_csv:
            writer = csv.writer(f_param_raw_csv)
            writer.writerow(cols)
            writer.writerows(params)

        with open(save_file_csv_time, 'w') as f_time_axis_csv:
            writer = csv.writer(f_time_axis_csv)
            writer.writerow(time_axis)
    ...
    # ############################## Begin/Continue Merge from start_idx ###############################################
    for free_param_counter in range(start_idx, unique_free_params.shape[0]):
        ...
        with open(save_file_csv, 'a') as f_save_csv:
            writer = csv.writer(f_save_csv)
            writer.writerow(free_params_avg_current)
        t2 = time.perf_counter()
