...
def get_axis_size(ax=None):
    """
     ax: matplotlib axis to measure
     Returns width, height in inches
    """
    if not ax: ax = plt.gca()
    bbox = ax.get_position().size  # [x, y]
...
def set_axis_size(width, height, padding=None, ax=None):
    """
    width, height in inches
    padding: [l, r, t, b] in inches
    ax: matplotlib axes to set size
    Adjusts size of axis and surrounding figure based on exact width and height in inches. Padding is conserved unless
    a new padding is provided.
    """

    if not ax: ax = plt.gca()
    bbox_init = ax.get_position()
    figw_init, figh_init = ax.figure.get_size_inches()
    if not padding:
        l_init = bbox_init.min[0]
        r_init = bbox_init.max[0]
        t_init = bbox_init.max[1]
        b_init = bbox_init.min[1]
        lpad = l_init * figw_init
        rpad = figw_init * (1 - r_init)
        tpad = figh_init * (1 - t_init)
        bpad = b_init * figh_init
    else:
        lpad, rpad, tpad, bpad = padding
    # Solve System
    figh = height + tpad + bpad
    figw = width + rpad + lpad
    left = 1 - (width + rpad) / figw
    right = left + (width / figw)
    bottom = 1 - (height + tpad) / figh
    top = bottom + (height / figh)

    ax.figure.set_size_inches(figw, figh)
    bbox = matplotlib.transforms.Bbox.from_extents(left, bottom, right, top)
    ax.set_position(bbox)


def round_sig_decimal(x):
    assert x >= 0, 'x must be greater than or equal to zero'
    y = x - int(x)
    dec_place = int(np.ceil(np.log10(1 / y)))
    rounded = np.round(x, dec_place)
    if rounded >= x:
        return rounded
    else:
        return rounded + 10**(-dec_place)


def hsv_darken_cmap(cmap, value=1.):
    """
    Alters a colormap brightness by factor value. Positive values increase brightness.
    :param cmap: Colormap
    :param value: Brightness factor
    :return: LinearSegmentedColormap in RGB values after brightness alteration
    """
...
def plot_movie_step(coords, time_ind, label_ind, lines, time_axis, recording_slice, min_v, max_v, save_folder, save, show, trace_data=None):
...
parula_map = matplotlib.colors.LinearSegmentedColormap.from_list('parula', cm_data)
