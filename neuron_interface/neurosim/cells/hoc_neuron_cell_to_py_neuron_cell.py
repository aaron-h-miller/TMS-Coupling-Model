...
def convert_hoc_to_python_cell(cell_type: str) -> Tuple[str, str]:
    ...
    ----------
    cell_type : str
        The name of the neuron model to convert

    Returns
    -------
    Tuple[str, str]
        The file name and the class name of the new Python neuron model
    """
    path = nrn_path.joinpath(f"cells/cells_hoc/{cell_type}").absolute()
    output = nrn_path.joinpath(f'cells/{cell_type.lower().replace("-", "_")}.py')
    morphology_path = f"str(pathlib.Path(neuron_interface.neurosim.__file__).parent.joinpath('cells/cells_hoc/{cell_type}/morphology/').resolve())"
    ...
    for section_type in section_types:
        ...
        apply_biophysics += "\n"
    segment_attributes = re.findall(
        'distribute\\(\\$o1\\.([^,]*),"([^"]*)","([^"]*)"', biophysics
    )
    d = defaultdict(list)
    ...
    with open(output, "w") as f, open(template_file_path, "r") as template:
        ...
        template_cell = template_cell.replace(
            "NeuronCellTemplate", f'{cell_type.replace("-", "_")}'
        )
...
if __name__ == "__main__":
    nrn_path = pathlib.Path(neuron_interface.neurosim.__file__).parent
    neuron_cell_names = [
        folder_paths.name
