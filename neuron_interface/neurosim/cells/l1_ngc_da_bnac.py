...
class L1_NGC_DA_bNAC(NeuronCell):
    ...
    def from_id(
        ...
        return cls(
            str(
                list(
                    pathlib.Path(str(pathlib.Path(neuron_interface.neurosim.__file__).parent.joinpath('cells/cells_hoc/L1_NGC-DA_bNAC/morphology/').resolve()))
    ...
    def get_morphology_ids():
        return sorted(
            [
                int(f.name)
                for f in pathlib.Path(str(pathlib.Path(neuron_interface.neurosim.__file__).parent.joinpath('cells/cells_hoc/L1_NGC-DA_bNAC/morphology/').resolve())).iterdir()
                if f.is_dir()
            ]
        )
