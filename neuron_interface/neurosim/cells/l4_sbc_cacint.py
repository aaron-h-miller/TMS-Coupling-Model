...
class L4_SBC_cACint(NeuronCell):
    ...
    def from_id(
        ...
        return cls(
            str(
                list(
                    pathlib.Path(str(pathlib.Path(neuron_interface.neurosim.__file__).parent.joinpath('cells/cells_hoc/L4_SBC_cACint/morphology/').resolve()))
    ...
    def get_morphology_ids():
        return sorted(
            [
                int(f.name)
                for f in pathlib.Path(str(pathlib.Path(neuron_interface.neurosim.__file__).parent.joinpath('cells/cells_hoc/L4_SBC_cACint/morphology/').resolve())).iterdir()
                if f.is_dir()
            ]
        )
    ...
    def apply_biophysics(self):
        ...
        for dend_section in self.dend:
            for dend_segment in dend_section.allseg():
                ...
                dend_segment.gIhbar_Ih = (-0.869600 + 2.087000*math.exp((dend_segment_distance-0.000000)*0.003000))*0.000023
        ...
        for apic_section in self.apic:
            for apic_segment in apic_section.allseg():
                ...
                apic_segment.gIhbar_Ih = (-0.869600 + 2.087000*math.exp((apic_segment_distance-0.000000)*0.003000))*0.000023
