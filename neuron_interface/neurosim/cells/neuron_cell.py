...
from .cell_modification_parameters.maxh_modification_parameters import (
    MaxhModificationParameters,
)


class NeuronCell:
    """A NEURON neuron defintion.
    ...
    --------------------------
    morphology_path : str
        Path to the morphology file (.asc) of the cell
    morphology_id : int
        The ID of this specific neuron morphologie
    modification_parameters : CellModificationParameters, optional
        Parameters for cell modification e.g. scaling and myelination, by default None
    variation_seed : int, optional
        The seed used to vary the cell on load, by default None


    Attributes
    ----------------------------------
    all: list[nrn.Section] (Nx1)
        Contains all sections that belong to this cell
    axon: list[nrn.Section] (Nx1)
        Contains all axonal sections that belong to this cell
    myelin: list[nrn.Section] (Nx1)
        Contains all myelinated axonal sections that belong to this cell (Subset of axon)
    node: list[nrn.Section] (Nx1)
        Contains all axonal node sections that belong to this cell (Subset of axon)
    unmyelin: list[nrn.Section] (Nx1)
        Contains all unmyelinated axonal sections that belong to this cell (Subset of axon)
    dend: list[nrn.Section] (Nx1)
        Contains all dendrite sections that belong to this cell
    soma: list[nrn.Section] (Nx1)
        Contains all somatic sections that belong to this cell
    apic: list[nrn.Section] (Nx1)
        Contains all apical sections that belong to this cell
    direction: npt.NDArray[np.float_] (1x3)
        The main cell direction pointing upwards from the soma
    variation_seed: int
        The seed for the random number generator to create a varied cell
    morphology_path: str
        Path to the morphology file (.asc) of the cell
    modification_parameters: CellModificationParameters
        Parameters for cell modification like scaling and myelination
    loaded: bool
        True if cell is currently loaded into the NEURON global space, False otherwise
    """
    ...
    def load(self, modification_parameters: CellModificationParameters = None) -> None:
        ...
        ----------
        modification_parameters : CellModificationParameters, optional
            Parameters for cell modification like scaling and myelination, by default None
        """
        if self.loaded:
            return
        ...
        self._scale_section_point_diameters(
            self.modification_parameters.apic_diameter_scaling_factor, self.apic
        )
        self._scale_section_point_diameters(
            self.modification_parameters.dend_diameter_scaling_factor, self.dend
        )
        self._scale_section_length(
            self.modification_parameters.dend_length_scaling_factor, self.dend
        )
        self._scale_section_point_diameters(
            self.modification_parameters.axon_diameter_scaling_factor, self.axon
        )

        self._rotate_x_90()

        if (
            self.modification_parameters.axon_modification_mode
            == AxonModificationMode.MYELINATED_AXON
        ):
    ...
    def _variegate(self, variation_seed: typing.Optional[int] = None) -> None:
        ...
        for section in self.axon + self.dend + self.apic:
            ...
            if len(section.children()) > 1:
                ...
                for child in section.children():
                    x = [
                        [s.x3d(i), s.y3d(i), s.z3d(i)]
                        for s in child.subtree()
                        for i in range(s.n3d())
                    ]
                    principal_comp = svd((np.array(x) - bif_pt).T)[0][
                        :, 0
                    ...
                    for subsec in child.subtree():
                        old_x = np.array(
                            [
                                [subsec.x3d(i), subsec.y3d(i), subsec.z3d(i)]
                                for i in range(subsec.n3d())
                            ]
                        )
                        new_x = (
                            Rotation.from_rotvec(angle * principal_comp).apply(
                                old_x - bif_pt
                            )
                            + bif_pt
                        )
    ...
    @staticmethod
    def _set_segment_count(section_list: list[nrn.Section], chunk_size: float) -> None:
        """Sets the segment count of each section in section_list so that each segment has a maximum length of chunk_size.
        For every chunk_size, 2 segments are added.

        Parameters
        ----------
        section_list : list[nrn.Section]
            The list of sections whose segment number should be updated
        chunk_size : float
            The chunk size in microns
        """
    ...
    @staticmethod
    def _scale_section_point_diameters(
        scale_factor: float, sections: list[nrn.Section]
    ) -> None:
        ...
        ----------
        scale_factor : float
            Scale factor for the point diameters
        sections : list[nrn.Section]
            The sections whose point diameters are supposed to be scaled
        """
        if not np.isclose(scale_factor, 1):
            for section in sections:
                for point_index in range(section.n3d()):
                    section.pt3dchange(
                        point_index, section.diam3d(point_index) * scale_factor
                    )

    @staticmethod
    def _scale_section_length(scale_factor: float, sections: list[nrn.Section]) -> None:
        """Scales the length of the given sections by the specified factor

        Parameters
        ----------
        scale_factor : float
            Scale factor for the section length
        sections : list[nrn.Section]
            The sections whose length is supposed to be scaled
        """
        if not np.isclose(scale_factor, 1):
            for section in sections:
    ...
    def _scale_soma_area(self, scale_factor: float) -> None:
        ...
        ----------
        scale_factor : float
            Scale factor for the soma area
        """
        if not np.isclose(scale_factor, 1):
            self.soma[0](0.5).diam = (
                scale_factor * self.soma[0](0.5).area() / (math.pi * self.soma[0].L)
            )
    ...
    def _myelinate_axon(self) -> None:
        ...
        if (
            self.axon[0].L
            >= self.MIN_PRE_MYELIN_AXON_LENGTH
            + self.modification_parameters.min_myelin_length
        ):
        ...
        (
            ...
            axon_section_myelin_section_count,
            unmyelin_section_count,
        ) = self._calculate_myelin_section_counts(axon_without_root)

        myelin = [
            h.Section(name=f"Myelin[{n}]", cell=self)
            for n in range(axon_section_myelin_section_count)
        ]
        nodes = [
            h.Section(name=f"Node[{n}]", cell=self)
            for n in range(axon_section_myelin_section_count)
        ]
        unmyelin = [
            h.Section(name=f"Unmyelin[{n}]", cell=self)
            for n in range(unmyelin_section_count)
        ]
        ...
        for i, axon_section in enumerate(axon_without_root):
            ...
            if axon_section_myelin_section_count == 0:
                ...
                for point_index in range(axon_section.n3d()):
                    unmyelin[unmyelinated_index].pt3dadd(
                        axon_section.x3d(point_index),
                        axon_section.y3d(point_index),
                ...
                axon_section.pt3dchange(
                    0,
                    axon_section_parent.x3d(axon_section_parent.n3d() - 1),
                ...
                myelin_length = (
                    axon_section.L
                    - axon_section_myelin_section_count * self.NODE_LENGTH
                ) / axon_section_myelin_section_count
                current_new_section = axon_section_parent
                current_section_start = 0
                point_index = 0
                for myelin_section_index in range(
                    axon_section_myelin_section_count * 2
                ):
                    ...
                    current_new_section.pt3dadd(
                        previous_section.x3d(previous_section.n3d() - 1),
                    ...
                    while (
                        point_index < axon_section.n3d()
                        and axon_section.arc3d(point_index) - current_section_start
                        < current_new_section_length
                    ):
                        current_new_section.pt3dadd(
                            axon_section.x3d(point_index),
                            axon_section.y3d(point_index),
                    ...
                    if point_index < axon_section.n3d():
                        ...
                        current_new_section.pt3dadd(
                            *self._interpolate_point_between(
                                length_left,
                                current_new_section,
                                current_new_section.n3d() - 1,
                                axon_section,
                                point_index,
                            )
                        )
    ...
    def _calculate_myelin_section_counts(
        self, axon_without_root: list[nrn.Section]
    ) -> tuple[list[int], int, int]:
        """Calculates how many myelin, node, unmyelin sections fit in each axon_without_root section.

        Parameters
        ----------
        axon_without_root : list[nrn.Section]
            The sections which length is used to determin the amount of myelin,
            node and unmyelin sections fit into them

        Returns
        -------
        tuple[list[int], int, int]
            A tuple containing a list of myelin/node section count for each axon_without_root section,
        ...
        for axon_section in axon_without_root:
            ...
            if axon_section(0).diam >= self.modification_parameters.min_myelin_diameter:
                axon_section_myelin_count += int(
                    axon_section.L / (myelin_length + self.NODE_LENGTH)
                )
    ...
    def _seperate_pre_myelin_axon(self) -> None:
        ...
        for i in range(self.axon[0].n3d()):
            if self.axon[0].arc3d(point_index) > self.MIN_PRE_MYELIN_AXON_LENGTH:
                new_axon_section.pt3dadd(
                    self.axon[0].x3d(point_index),
                    self.axon[0].y3d(point_index),
                    self.axon[0].z3d(point_index),
                    self.axon[0].diam3d(point_index),
                )
        ...
        new_axon_section.pt3dinsert(
            0,
            self.axon[0].x3d(self.axon[0].n3d() - 1),
    ...
    @staticmethod
    def _interpolate_point_between(
        distance: float,
        section_end: nrn.Section,
        ...
        ----------
        distance : float
            The distance the point is away from section_end point with index section_end_index
        section_end : nrn.Section
            The section containing the first point for the interpolation
        section_end_index : int
             The index to the first point of the interpolation in section_end
        section_start : nrn.Section
            The section containing the second point for the interpolation
        section_start_index : int
             The index to the first point of the interpolation in section_end
        ...
        -------
        tuple[float, float, float, float]
            A tuple containing the x,y,z coordinates as well as the diameter of the interpolated point
        """

        x_displacement = section_start.x3d(section_start_index) - section_end.x3d(
            section_end_index
        )
        y_displacement = section_start.y3d(section_start_index) - section_end.y3d(
            section_end_index
        )
        z_displacement = section_start.z3d(section_start_index) - section_end.z3d(
            section_end_index
        )
        point_distance = math.sqrt(
            x_displacement**2 + y_displacement**2 + z_displacement**2
        )

        return (
            section_end.x3d(section_end_index)
            + distance * x_displacement / point_distance,
            section_end.y3d(section_end_index)
            + distance * y_displacement / point_distance,
            section_end.z3d(section_end_index)
            + distance * z_displacement / point_distance,
    ...
    def disable_main_axon_furthest(self):
        ...
        main_axon_xy_radius = 500 * um

        min_axon_section = self.axon[0]
        min_axon_point_index = 0
        for section in self.axon + self.myelin + self.unmyelin + self.node:
            for point_index in range(section.n3d()):
                if section.z3d(point_index) < min_axon_section.z3d(
                    min_axon_point_index
                ):
                    axon_xy_distance = np.linalg.norm(
                        np.array(
                            [self.soma[0](0).x_xtra, self.soma[0](0).y_xtra]
                            - np.array(
                                [section.x3d(point_index), section.y3d(point_index)]
                            )
                        )
                    )
        ...
        min_axon_section(1).diam = 1000 * um
        ...
        for section in self.all:
            if section.has_membrane("xtra"):
                ...
                x_interpolated = np.interp(
                    segment_coordinates,
                    segment_geometries[:, 3],
                    segment_geometries[:, 0],
                )
                y_interpolated = np.interp(
                    segment_coordinates,
                    segment_geometries[:, 3],
                    segment_geometries[:, 1],
                )
                z_interpolated = np.interp(
                    segment_coordinates,
                    segment_geometries[:, 3],
                    segment_geometries[:, 2],
                )
                ...
                if section.nseg == 1:
                    segments[0].Dx_xtra = section.x3d(section.n3d() - 1) - section.x3d(
                        0
                    )
                    segments[0].Dy_xtra = section.y3d(section.n3d() - 1) - section.y3d(
                        0
                    )
                    segments[0].Dz_xtra = section.z3d(section.n3d() - 1) - section.z3d(
                        0
                    )
                else:
        ...
        ** contains parent side bifurcation point (1) **
        ...
        for section in self.all:
            ...
            if section.has_membrane("pas"):
                section_lambda = 50 * math.sqrt(
                    section(1).diam / (section(1).g_pas * section.Ra)
                )
            ...
            if len(parent_children) == 1:
                ...
                if len(children) == 0:
                    section(1).type_xtra = (
                        5 if section_length <= section_lambda else 2
                    )
                elif len(children) >= 2:
                    section(1).type_xtra = (
                        7 if section_length <= section_lambda else 4
                    )
    ...
    def get_segment_coordinates(self) -> npt.NDArray[np.float_]:
        """Returns a list of all segment center coordinates.

        Returns
        -------
        npt.NDArray[np.float_] (segment_count x 3)
            A list of all segment center coordinates in the format [x,y,z] in um
        """
        segment_positions = []
        for section in self.all:
            if section.has_membrane("xtra"):
                for segment in section:
                    segment_positions.append(
                        [segment.x_xtra, segment.y_xtra, segment.z_xtra]
                    )
    ...
    def display_2d_matplotlib(
        ...
        Parameters
        ----------
        path : typing.Union[str, pathlib.Path], optional
            Path to save the plot to. If None: shows plot blocking, by default None
        include_diameter : bool, optional
             If True, the diameter will be displayed. If False, all sections will have the same diameter, by default False
        rotated_90 : bool, optional
            If True, rotates the neuron by 90 degrees azimuthal, by default False
        plot_grid : bool, optional
            If False, the grid will not be plotted, by default True
        background_transparent : bool, optional
            If True, the background will be plotted transparent, by default False
        y_range : list[float] (2), optional
            The y range to use for the plot. If None: takes the max and min y coordinates as the range, by default None
        """
        ...
        for i, section in enumerate(z_sorted_sections):
                ...
                if rotated_90:
                    if include_diameter:
                        soma_circle = plt.Circle(
                            (self.soma[0](0.5).y_xtra, self.soma[0](0.5).z_xtra),
                            self.soma[0].L / 2,
                            color=color[section_type],
                            zorder=i,
                        )
                    else:
                        soma_circle = plt.Circle(
                    ...
                    if include_diameter:
                        soma_circle = plt.Circle(
                            (self.soma[0](0.5).x_xtra, self.soma[0](0.5).z_xtra),
                            self.soma[0].L / 2,
                            color=color[section_type],
                            zorder=i,
                        )
            ...
            if include_diameter:
                for point_index in range(section.n3d() - 1):
                    ortogonal = np.array(
                        [x_coordinates[point_index + 1], y_coordinates[point_index + 1]]
                    ) - np.array(
                    ...
                    s1 = np.array(
                        [x_coordinates[point_index], y_coordinates[point_index]]
                    ) + ortogonal * (diameter[point_index] / 2)
                    s2 = np.array(
                        [x_coordinates[point_index], y_coordinates[point_index]]
                    ) + ortogonal * -(diameter[point_index] / 2)
                    e1 = np.array(
                        [x_coordinates[point_index + 1], y_coordinates[point_index + 1]]
                    ) + ortogonal * (diameter[point_index + 1] / 2)
                    e2 = np.array(
                        [x_coordinates[point_index + 1], y_coordinates[point_index + 1]]
                    ) + ortogonal * -(diameter[point_index + 1] / 2)
                    polygon = plt.Polygon(
                        [s1, s2, e2, e1], True, facecolor=color[section_type], zorder=i
                    )
                ...
                plt.plot(
                    x_coordinates,
                    y_coordinates,
                    c=color[section.name().split(".")[-1].split("[")[0]],
                    lw=3,
                )
        legend_lines = [
            Line2D(
    ...
    def write_stl(self, path: typing.Union[str, pathlib.Path]) -> None:
        ...
        for section in self.all:
            ...
            for point_index in range(section.n3d() - 1):
                if np.allclose(
                    [
                        section.x3d(point_index),
                        section.y3d(point_index),
                        section.z3d(point_index),
                        section.diam3d(point_index),
                    ],
                    [
                        section.x3d(point_index + 1),
                        section.y3d(point_index + 1),
                        section.z3d(point_index + 1),
                        section.diam3d(point_index + 1),
                    ],
                ):
                    continue
        ...
        with open(pathlib.Path(path), "w") as f:
            for triangles in triangles_all:
                ...
                for triangle in triangles_all[triangles]:
                    ...
                    f.write(
                        f"\t\tvertex {triangle[0][0]} {triangle[0][1]} {triangle[0][2]}\n"
                    )
                    f.write(
                        f"\t\tvertex {triangle[1][0]} {triangle[1][1]} {triangle[1][2]}\n"
                    )
                    f.write(
                        f"\t\tvertex {triangle[2][0]} {triangle[2][1]} {triangle[2][2]}\n"
                    )
                    f.write(f"\tendloop\n")
                    f.write(f"endfacet\n")
